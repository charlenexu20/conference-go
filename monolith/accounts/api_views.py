...
# the encoder that creates the JSON for your microservice
...
def send_account_data(account):
    ...
    # create a message and turn it into json so we can send it
    message = json.dumps(account, cls=AccountInfoModelEncoder)
    # publish the message to the exchange - no routing key because it publishes to all queues
